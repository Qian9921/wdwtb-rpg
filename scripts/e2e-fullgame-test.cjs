...
(async()=>{
  ...
  let r=await doChain('test_c1','zhao');
  t('t_c1 交付 12%', r.done && r.progress===12);
  r=await doChain('test_c2','lin');
  t('t_c2 交付 32% → 触发25%里程碑(pendingAct=2)', r.done && r.progress===32 && r.pending===2);
  ...
  // c3(52%→50里程碑→act3)
  r=await doChain('test_c3','zhao');
  t('t_c3 交付 52% → pendingAct=3', r.done && r.progress===52 && r.pending===3);
  await ev(()=>{const w=window.__game.scene.getScene('WorldScene');
    const chen=w.npcs.find(n=>n.id==='senior'); w._interactSenior(chen);});
  await sleep(900);
  ...
  // c4(76%→75里程碑→act4)
  r=await doChain('test_c4','zhao');
  t('t_c4 交付 76% → pendingAct=4', r.done && r.progress===76 && r.pending===4);
  await ev(()=>{const w=window.__game.scene.getScene('WorldScene');
    const chen=w.npcs.find(n=>n.id==='senior'); w._interactSenior(chen);});
  await sleep(900);
  ...
  // c5(100%→100里程碑→act5)
  r=await doChain('test_c5','vet');
  t('t_c5 交付 100% → pendingAct=5', r.done && r.progress===100 && r.pending===5);
  await ev(()=>{const w=window.__game.scene.getScene('WorldScene');
    const chen=w.npcs.find(n=>n.id==='senior'); w._interactSenior(chen);});
  await sleep(900);
